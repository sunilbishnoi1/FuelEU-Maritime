...
export const AvailableShips: React.FC<AvailableShipsProps> = ({
  ...
  return (
    <Card className="p-6">
      <h3 className="text-lg font-semibold mb-4 text-secondary-900">Available Ships</h3>
      <div className="space-y-3 max-h-96 overflow-y-auto">
        {ships.length === 0 ? (
          <p className="text-muted-foreground">No ships available for pooling</p>
        ) : (
          ships.map((ship) => (
            <div key={ship.ship_id} className="flex items-center justify-between p-3 rounded-lg hover:bg-primary-50 transition-colors border border-border">
              <label htmlFor={`ship-${ship.ship_id}`} className="flex items-center gap-3 cursor-pointer flex-1">
                ...
                />
                <span className="font-medium text-secondary-800">Ship {ship.ship_id}</span>
              </label>
              <span className={`text-xs font-mono ml-2 ${ship.adjusted_cb_gco2eq >= 0 ? 'text-primary-700' : 'text-destructive'}`}>
